...
def _ollama_suggest(instruction: str, column: str | None) -> Optional[Dict[str, Any]]:
    try:
        ...
        if obj and isinstance(obj.get("regex"), str):
            rep = _infer_replacement_quoted(instruction)
            _log(f"inferred replacement: {rep!r}")
            result = {
                "regex": obj["regex"],
...
def _openai_suggest(client, instruction: str, column: str | None) -> Optional[Dict[str, Any]]:
    try:
        ...
        if obj and isinstance(obj.get("regex"), str):
            ...
            if rep:                                            # NEW: only include when found
                result["replacement"] = rep
            return result
    except Exception as e:
...
# Handles the case where user says the regex and the replacement word
def _infer_replacement_quoted(text: str) -> str:
    if not text:
        return ""
    ...
    patterns = [
        rf"replace(?:\s+\w+){{0,6}}\s+(?:with|to|by)\s+{quote_group}",
        rf"mask(?:ed|ing)?(?:\s+\w+){{0,6}}\s+(?:with|as)\s+{quote_group}",
        rf"set(?:\s+\w+){{0,6}}\s+to\s+{quote_group}",
    ]
    lower = text.strip()
    for p in patterns:
        m = re.search(p, lower, flags=re.IGNORECASE | re.DOTALL)
        if m and m.group(2):
